#!/usr/bin/env python3
"""Task 14: 6-Minute Full Validation Run — comprehensive validation.

Executes final progressive validation run with:
  - Duration: 360 seconds (6 minutes)
  - Scenarios: 50 (full catalogue)
  - Facts per user: 40+ (maximum)
  - Validates: stress test, complete audit logging + validation,
               full verification of Criterion G (audit completeness) and
               Criterion H (user isolation)

Output: reports/run_4_6min.json
"""
...
async def run_6min() -> dict:
    """Execute 6-minute validation run and return results."""
    run_number = 4
    duration_seconds = 360

    logger.info(f"Starting 6-minute validation run (run_number={run_number})...")
    ...
    async with EngineClient(base_url=settings.engine_api_base_url) as engine_client:
        ...
        report_path = reports_dir / "run_4_6min.json"
...
def main():
    """Entry point for 6-minute validation run."""
    logging.basicConfig(
        level=logging.INFO,
    ...
    try:
        result = asyncio.run(run_6min())
        logger.info(
            f"6-minute validation run COMPLETED: "
            f"{result['scenario_summary']['facts_played']} facts played, "
            f"{result['scenario_summary']['errors']} errors"
        )
        return 0
    except Exception as e:
        logger.error(f"6-minute validation run FAILED: {e}", exc_info=True)
        return 1
