#!/usr/bin/env python3
"""Task 13: 4-Minute Validation Run — extended validation (continued).

Executes third progressive validation run with:
  - Duration: 240 seconds
  - Scenarios: ~50 (full catalogue)
  - Facts per user: ~25-35
  - Validates: audit completeness (Criterion G), user isolation (Criterion H),
               longer observation window for TTL scenarios

Output: reports/run_3_4min.json
"""
...
async def run_4min() -> dict:
    """Execute 4-minute validation run and return results."""
    run_number = 3
    duration_seconds = 240

    logger.info(f"Starting 4-minute validation run (run_number={run_number})...")
    ...
    async with EngineClient(base_url=settings.engine_api_base_url) as engine_client:
        ...
        report_path = reports_dir / "run_3_4min.json"
...
def main():
    """Entry point for 4-minute validation run."""
    logging.basicConfig(
        level=logging.INFO,
    ...
    try:
        result = asyncio.run(run_4min())
        logger.info(
            f"4-minute validation run COMPLETED: "
            f"{result['scenario_summary']['facts_played']} facts played, "
            f"{result['scenario_summary']['errors']} errors"
        )
        return 0
    except Exception as e:
        logger.error(f"4-minute validation run FAILED: {e}", exc_info=True)
        return 1
