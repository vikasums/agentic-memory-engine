#!/usr/bin/env python3
"""Task 12: 1-Minute Validation Run — smoke testing.

Executes the first progressive validation run with:
  - Duration: 60 seconds
  - Scenarios: ~50 (full catalogue)
  - Facts per user: ~8-10
  - Validates: smoke test, basic audit completeness, user isolation

Output: reports/run_1_1min.json
"""
...
async def run_1min() -> dict:
    """Execute 1-minute validation run and return results."""
    run_number = 1
    duration_seconds = 60

    logger.info(f"Starting 1-minute validation run (run_number={run_number})...")
    ...
    async with EngineClient(base_url=settings.engine_api_base_url) as engine_client:
        ...
        report_path = reports_dir / "run_1_1min.json"
...
def main():
    """Entry point for 1-minute validation run."""
    logging.basicConfig(
        level=logging.INFO,
    ...
    try:
        result = asyncio.run(run_1min())
        logger.info(
            f"1-minute validation run COMPLETED: "
            f"{result['scenario_summary']['facts_played']} facts played, "
            f"{result['scenario_summary']['errors']} errors"
        )
        return 0
    except Exception as e:
        logger.error(f"1-minute validation run FAILED: {e}", exc_info=True)
        return 1
